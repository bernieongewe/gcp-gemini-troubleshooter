...
`gcp-gemini-troubleshooter` is an experimental Gemini CLI extension designed for Cloud Architects, and Power Users. It acts as a reasoning engine that leverages the Gemini API to diagnose complex issues across the GCP ecosystem without modifying your environment. I invite you to test, tune and extend this.
...
3. **Advise:** Provide `gcloud` commands or architectural changes for the user to execute manually.

*It will never execute `gcloud compute instances delete` or any state-changing command.*

## ⚠️ Prerequisites & Permissions
This tool relies on the **`gcloud` CLI** to fetch diagnostic data.
* **IAM Permissions:** It uses *your* local authenticated `gcloud` session. You must have sufficient permissions (e.g., `Viewer`, `Security Viewer`) for the resources you are diagnosing.
